...
import com.mica.viva.understanding.constant.StageType;
...
public class Understanding {
    ...
    private Parameter tempParam = null;
            ...
            switch (keyword.type) {
                ...
                case ToContact :
                    ...
                    if (keyword.type == KeywordType.ContactName) {
                        tempParam = new Parameter("RECEIVER", keyword.keyword);
                    }
                    break;
                default : 
                    keyword = kt.next();
    ...
    private void compileMessage() {
        ...
        function = new Function("SENDING_SMS", "SMS");
        keyword = kt.next();
    ...
    private void compileWaitContactHadContent() {
        ...
        if (keyword.type == KeywordType.ContactName) {
            messageContent = temp;
            function.addParameter(new Parameter("RECEIVER", keyword.keyword));
    ...
    private void compileHadContentNoContact() {
        ...
        if (keyword.type == KeywordType.End) { //System.out.println(keyword.type);
            function.addParameter(new Parameter("CONTENT", messageContent));
        } else {
            if (keyword.type == KeywordType.ToContact) {
                keyword = kt.next();
            }
        ...
        while (true) {
            switch (keyword.type) {
                case End :
                    return;
                case Place :
                	function.addParameter(new Parameter(WEATHER_PLACE_PARAM_CODE, keyword.keyword));
                    compileWeatherWithPlace();
                    break;
                case Time :
                	function.addParameter(new Parameter(WEATHER_TIME_PARAM_CODE, keyword.keyword));
